...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreeEntry temp = null;// 遍历出一个entry
	...
	private BTreeLeafPage reverseFindLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// reverseIterate through the entries in the internal page and compare the entry value to the provided key value
		Iterator<BTreeEntry> it=internalPage.reverseIterator();
		BTreeEntry temp = null;// 遍历出一个entry
		while(it.hasNext()) {// 遍历当前节点所有的entry寻找f
			temp=it.next();
			// recurse on the right-most child every time in order to find the right-most leaf page
			if(f==null)
				return findLeafPage(tid,dirtypages,temp.getRightChild(),perm,f);
			if(temp.getKey().compare(Op.LESS_THAN_OR_EQ,f))
				// return the first (right) leaf page
				return findLeafPage(tid,dirtypages,temp.getRightChild(),perm,f);
		}
		// 找到了最后一个entry了，那只能是往左边走
		return findLeafPage(tid,dirtypages,temp.getLeftChild(),perm,f);	
	}
	 ...
	 * pointers as needed.  
	 * 往父节点传输entry，key是第一个右半节点的第一个tuple对应的field(copy up),需要更新叶节点的兄弟指针和父指针
	 * 
	 * 返回插入new tuple的page
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
        ...
        // 1.Split the leaf page by adding a new page on the right of the existing
		...
		// moving a subset of tuples/entries from a page to its right sibling
		int numberOfTuples=page.getNumTuples();
		Iterator<Tuple> it=page.reverseIterator();
		for(int i=0;i<numberOfTuples/2;i++) {
			Tuple temp=it.next();
			page.deleteTuple(temp);// 从原页面删除该tuple
			rightPage.insertTuple(temp);// 放到右兄弟去
		}
		
		// 2.Copy the middle key up into the parent page, 
		// and recursively split the parent as needed to accommodate the new entry.  
		// getParentWithEmtpySlots() will be useful here.  
		
		// interact with leaf and internal pages using .iterator() to iterate through the tuples/entries in each page!
		...
		// update the parent pointers of all the children that were moved->updateParentPointers()
		updateParentPointers(tid,dirtypages,parent);
		
		
		// 3.Don't forget to update the sibling pointers of all the affected leaf pages.
		
		// 原节点右节点指针更新
		BTreePageId oldRightPageId=page.getRightSiblingId();
		if(oldRightPageId!=null) {// 可能就已经是最右边了
			// 更新左右指针
			BTreeLeafPage oldRightPage=(BTreeLeafPage)getPage(tid,dirtypages,oldRightPageId,Permissions.READ_ONLY);
		...
		// 4.Return the page into which a tuple with the given key field should be inserted.
        // 看field是插入到哪一半
		if(field.compare(Op.LESS_THAN, midKey))// 比midKey小，则插入到原来那一页中
			return page;
		else
			return rightPage;
	 ...
	 * 将新的entry push up！新entry的child pointers指向分裂的子节点
	 * 
	 ...
	 * 让父节点做好接受new entry的准备->new root/splitting/locking&returning
	 *
	 * @param tid - the transaction id
	 * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
	...
	private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if(parentId.pgcateg() == BTreePageId.ROOT_PTR) {
			...
			// 获取之前的根节点id，后面生成原根节点，并让他的父节点指向新根节点
	...
	private void updateParentPointers(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage page) 
			throws DbException, IOException, TransactionAbortedException{
		Iterator<BTreeEntry> it = page.iterator();// 遍历当前节点每一个entry
		BTreePageId pid = page.getId();
		BTreeEntry e = null;
		while(it.hasNext()) {
			e = it.next();// 由entry得到孩子节点pageId
	 ...
	 * 与heapFile不同的是，可能返回许多page，因为分裂过程会使得很多页变脏
	 * 
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<Tuple> itOfRight=rightPage.iterator();
		while(itOfRight.hasNext()) {
			Tuple tempTuple=itOfRight.next();
			// 先删除再插入
		...
		if(rightPage.getRightSiblingId()!=null) {
			BTreeLeafPage rightRight=(BTreeLeafPage)getPage(tid,dirtypages,rightPage.getRightSiblingId(),
					Permissions.READ_WRITE);
			// 右右兄弟的左指针也得设置，所以得获取该节点
			rightRight.setLeftSiblingId(leftPage.getId());
			leftPage.setRightSiblingId(rightRight.getId());		
		}
		else
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> itOfRight=rightPage.iterator();
		// 删除parentEntry前得复制下来-> 设置孩子指针
		BTreeEntry pulledDown=new BTreeEntry(parentEntry.getKey(),leftPage.reverseIterator().next().getRightChild(),
				rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(pulledDown);
		while(itOfRight.hasNext()) {
			BTreeEntry tempEntry=itOfRight.next();
...
//		}
	 ...
	 * add by tang
	 * 
	 * Get an iterator for all tuples in this B+ tree file in reverseSorted order. This method 
	 * will acquire a read lock on the affected pages of the file, and may block until 
	 * the lock can be acquired.
	 * 
	 * @param tid - the transaction id
