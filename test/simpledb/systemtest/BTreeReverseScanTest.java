...
public class BTreeReverseScanTest extends SimpleDbTestBase {
    ...
    private static class TupleComparator implements Comparator<ArrayList<Integer>> {
    	...
    	public int compare(ArrayList<Integer> t1, ArrayList<Integer> t2) {// TODO:reverse
            int cmp = 0;
            // 调用的是collection的sort函数，原本的是：小->-1，放前面；大->1，放后面；相等的不变，稳定排序！
            // 现在需要reverse：小的->1，放后面...但有个问题：稳定排序
            // 用户请求才用：归并排序&插入排序，判断>0则swap
            // 默认使用的是TimSort&binarySort，判断条件是<0
            // 相等的元素，先进来的reverse之后要排到后面->等于的情况也需要进行交换位置！
    ...
    @Test public void testRewind() throws IOException, DbException, TransactionAbortedException {
        ...
        Collections.sort(tuples, new TupleComparator(keyField));// TODO：排序，稳定排序！
